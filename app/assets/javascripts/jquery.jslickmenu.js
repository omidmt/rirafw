...
(function($){
	$.fn.jSlickmenu = function(options) {
		...
		return this.each(function() {
			...
			{
				var wdt = (options.leftmargin + options.borderhover + options.css3borderradius) * $("#slickmenu ul li").length;
				$("#slickmenu").css({  'margin': '0 auto'});
                // 'width': wdt,
			}
			...
			for(var i = 0; i < $("#slickmenu ul").children().size(); i++) {
				...
				li_item.children().children().css({
					...
					'-khtml-border-radius': options.css3borderradius + 'px',
			...
			$("#slickmenu ul li a img").hover(function(){
				...
				$(this)
					.stop()
					.before('<div class="infobox pie">' + $(this).attr("alt") + '</div>')
					.css({
...
//$("#slickmenu ul li a img").each(function()
//{
//    $(this).before( '<div class="infobox pie">' + $(this).attr("alt") + '</div>')
//        .css({
//            '-webkit-box-shadow' : '#999 5px 10px 10px',
//            '-moz-box-shadow' : '#999 5px 10px 10px',
